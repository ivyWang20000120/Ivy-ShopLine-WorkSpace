# Lingxing ERP Inventory Sync — Privacy Policy

This policy applies only to services provided by Lingxing ERP Inventory Sync (hereinafter "the App" or "we").

Last updated : 2026-05-30
Effective date : 2026-05-30
Operator : Chengdu Silk Software Co., Ltd. (成都思而科软件有限公司)
Official website : https://www.silksoftware.com.cn/

Welcome to Lingxing ERP Inventory Sync (hereinafter "the App" or "we"). The App is designed for Shopline merchants and provides the following main capabilities:

- **Warehouse mapping configuration**: merchants configure the mapping between Lingxing ERP warehouses and Shopline inventory locations.
- **Inventory sync**: automatically polling Lingxing ERP inventory change records and syncing the latest available quantity to the corresponding Shopline inventory location.
- **Return processing**: monitoring Shopline return webhooks and triggering inventory restoration in Lingxing ERP.
...
- data synchronization required to read warehouse inventory, receive order events, and update Shopline inventory through Shopline.
...
If you are a merchant's customer (an end consumer) and wish to exercise rights related to orders or inventory, we generally recommend contacting the merchant first; where permitted by applicable law, we will assist the merchant as needed.
...
- product ID, name, SKU/variants, images, categories
- fields required to match order line items to warehouse mapping rules

**(3) Orders, fulfillment, and inventory data**

- order number, order status, fulfillment/shipping status
- line items, quantities, product identifiers, and amounts
- inventory levels and warehouse location data required for sync
- return event data from Shopline webhooks

**(4) Configuration**

- warehouse-to-location mapping, sync schedule, enabled/disabled status
...
### 4.1 Inventory sync and Shopline integration

- enable merchants to create, edit, or delete warehouse-location mapping rules
- poll Lingxing ERP inventory change records and sync to Shopline inventory locations
- implement idempotency to prevent duplicate syncs for the same inventory change
- process return webhooks to trigger inventory restoration in Lingxing ERP
...
- **Service providers**: we may share necessary data with providers such as Shopline, Lingxing ERP, and cloud/hosting/database/CDN/monitoring/logging/email vendors
- **At your direction**: when you use the App, we process and transmit data to Shopline and Lingxing ERP within the necessary scope to implement inventory sync and return processing operations
...
Inventory sync logs and return records may be retained as needed for consistency and dispute handling.

---

## 9. International Transfers

Information may be transferred to, stored in, or processed in countries or regions outside your jurisdiction (for example: regions where our hosting providers, Shopline, and Lingxing ERP process data). We apply reasonable safeguards as required by applicable law.
...
The App relies on Shopline and Lingxing ERP and may link to other third-party sites or services; each has its own privacy policy. Please review their notices before use.
